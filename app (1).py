...
<style>
    ...
    .subtitle   { font-size:1rem; color:#555; text-align:center; margin-bottom:0.5rem; }
...
# ═══════════════════════════════════════════════════════════
# URLs GitHub RAW — données chargées automatiquement
# ═══════════════════════════════════════════════════════════
GITHUB_USER = "moribanouho"
GITHUB_REPO = "analyse-ppi-gappi"
URL_CSV     = f"https://raw.githubusercontent.com/{GITHUB_USER}/{GITHUB_REPO}/main/HSFinalSIVF_communities_only.csv"
URL_TXT     = f"https://raw.githubusercontent.com/{GITHUB_USER}/{GITHUB_REPO}/main/bestSample.txt"

# ═══════════════════════════════════════════════════════════
# FONCTIONS
# ═══════════════════════════════════════════════════════════
@st.cache_data(show_spinner="⏳ Chargement du réseau PPI depuis GitHub...")
def charger_reseau_auto():
...
@st.cache_data(show_spinner="⏳ Chargement des communautés depuis GitHub...")
def charger_communautes_auto():
...
def dessiner_graphe(G, met, titre):
    ...
    if hub and hub in pos:
        ...
        nx.draw_networkx_nodes(G, pos, nodelist=[hub], node_color='red',
                               node_size=tailles[idx]+200, alpha=1.0, ax=ax)
...
def dessiner_histogrammes(moy, num):
    fig, axes = plt.subplots(1, 3, figsize=(14, 4))
    fig.suptitle(f'Communauté n°{num} — Distributions', fontsize=12, fontweight='bold')
    ...
    if moy['liste_sim']:
        ...
        axes[1].axvline(x=moy['AVGSIM'], color='red', linestyle='--', linewidth=2,
                        label=f"AVGSIM={moy['AVGSIM']:.3f}")
    ...
    if moy['liste_int']:
        ...
        axes[2].axvline(x=moy['AVGInteraction'], color='red', linestyle='--', linewidth=2,
                        label=f"AVGInt={moy['AVGInteraction']:.0f}")
...
# ═══════════════════════════════════════════════════════════
st.markdown('<div class="main-title">🧬 Analyse des Communautés de Gènes — Réseaux PPI</div>',
            unsafe_allow_html=True)
st.markdown('<div class="subtitle">Master 1 IES-D3S · Université Paris Nanterre · GA-PPI-Net · Soumahoro Moriba Nouho</div>',
            unsafe_allow_html=True)
st.markdown('<div class="subtitle">Basé sur les travaux de la thèse de <b>Marwa Ben M\'barek (2019)</b> — Université Paris Nanterre</div>',
            unsafe_allow_html=True)
st.divider()

# Chargement auto silencieux
try:
    df_ppi, dict_ppi = charger_reseau_auto()
    communautes      = charger_communautes_auto()
    st.success(f"✅ **{len(communautes)} communautés** disponibles · **{len(df_ppi):,} interactions** chargées")
...
with st.sidebar:
    ...
    seuil_interaction = st.slider("Seuil interaction (∇I)", 100, 900, 150, 50,
                                   help="Score STRING minimum pour tracer un lien")
    st.divider()
    mode = st.radio("Mode d'analyse",
                    ["🔬 Analyse d'une communauté",
                     "⚖️ Comparaison de deux communautés"])
    st.divider()
    st.markdown("""
    **À propos**

    Cette application analyse les communautés de gènes détectées par **GA-PPI-Net** dans le réseau
    PPI *Homo sapiens*.

    Travaux basés sur la thèse de **Marwa Ben M'barek (2019)** — *Détection de communautés dans
    les grands réseaux : Application aux réseaux d'interactions de gènes* — Université Paris Nanterre.

    **Fitness :** `F(S) = 0.5×AVGSIM + 0.5×AVGInt/1000`

    ---
    🔗 [Code source GitHub](https://github.com/moribanouho/analyse-ppi-gappi)
    """)
...
if "Analyse" in mode:
    ...
    num = st.selectbox(
        ...
        format_func=lambda i: f"Communauté {i+1}  ({len(communautes[i])} gènes)  — Hub pressenti : {communautes[i][0]}"
    )
    ...
    if st.button("🚀 Lancer l'analyse", type="primary", use_container_width=True):
        ...
        dessiner_histogrammes(moy, num+1)

        st.markdown('<div class="section-header">🔬 Validation biologique — Outil DAVID</div>',
                    unsafe_allow_html=True)
        st.markdown("""
        Copiez la liste ci-dessous et collez-la sur
        **[https://davidbioinformatics.nih.gov](https://davidbioinformatics.nih.gov)**
        → *Start Analysis* → coller les gènes → *OFFICIAL_GENE_SYMBOL* → *Homo sapiens*
        → *Submit List* → *Functional Annotation Chart*
        """)
...
# ═══════════════════════════════════════════════════════════
    ...
    with cs2:
        numB = st.selectbox("Communauté B", list(range(len(communautes))),
                            format_func=lambda i: f"Communauté {i+1} ({len(communautes[i])} gènes)",
                            index=1, key='B')
